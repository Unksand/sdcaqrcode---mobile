...
import { enter, arrowUpCircleOutline } from "ionicons/icons";
...
const Booksearch = (props: RouteComponentProps) => {
  ...
  const fetchBooks = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost/login/index.php/Booksload/search_books?query=${searchQuery}&is_borrowed=0`,
        {
          method: "GET",
        }
      );
  ...
  const borrowBook = async (bookId: number) => {
    try {
      ...
      const response = await fetch(
        `http://localhost/SDCAQrCode/index.php/Booksload/borrow_book/${bookId}`,
        {
          method: "POST",
          headers: {
  ...
  const fetchBorrowedBooks = async () => {
    try {
      ...
      const response = await fetch(
        "http://localhost/SDCAQrCode/index.php/Booksload/borrowed_bookss",
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const fetchBookByQRCode = async (qrcode: string) => {
    try {
      ...
      const response = await fetch(
        `http://localhost/login/index.php/Booksload/get_book_by_qrcode?qrcode=${qrcode}`,
        {
          method: "GET",
        }
      );
  ...
  return (
    <IonPage>
      ...
      <IonContent ref={contentRef} className="ion-padding book-content">
          ...
          <IonGrid>
            ...
            <IonRow>
              <IonCol size="13">
                ...
                />
                <IonButton shape="round" onClick={fetchBooks}>
                  Search
                </IonButton>
              </IonCol>
            </IonRow>
            ...
            <IonRow>
              <IonCol size="8">
                <IonButton
                  ...
                  expand="full"
                  onClick={() =>
                    licenseInitialized && props.history.push("scanner")
                  }
                >
